...
### Page layout
1. `complete - meets requirement` For screens wider than 1336px, page content* will be contained to the central 1136px of the screen width.
    1. `complete - meets requirement` The margins (the space to the left and right of the central 1136px) will not contain any page content*.
    
2. `complete - meets requirement` For screens narrower than 1337px, the left and right page margins will be 6% of the screen width.
    1. `complete - meets requirement` The margins will not contain any page content*.

3. `complete - meets requirement (48px margin below navbar, 48px margin from bottom of screen)` For all screen widths, the top and bottom page margins will be 48px.
    1. `complete - meets requirement` The margins will not contain any page content*.

4. `complete - meets requirement` Page headers should always be positioned at the top of the page content area, and all other content should be arranged beneath the header.
...
### Navbar
1. `complete - meets requirement` The navbar will be visible on every page on the website, and will be fully interactive as long as a modal is not open.

2. `revised - we decided that having a fixed navbar is not valuable as a v1.0 requirement, since pages will rarely extend beyond the desktop screen height (we are not prioritizing mobile responsiveness).` The navbar will always be fixed to the top of the page, ~~even when the user scrolls down the page.~~

3. `complete - meets requirement` The navbar will have a height of 136px and a width of 100% of the screen width.

4. `complete - meets requirement` All content (defined as elements with a functional purpose) in the navbar will be vertically centered.
    1. `complete - meets requirement` The Bonito logo will be left-aligned within the navbar’s content area.
    2. `complete - meets requirement` The primary links will be right-aligned and vertically centered within the navbar’s content area, in the order: 1) “Find Ingredients, 2) “About”, and 3) “Sign In” OR “Signed in as [user’s first name]”
        1. `revised - we decided that varying link spacing, based on screen width, looks better visually and makes the application easier to use` Each successive link will be ~~spaced 60px apart from the previous link.~~ _equally spaced apart, and will not overlap each other_.
...
6. `complete - meets requirement` If the user is not signed into Bonito via Google, the right-most link in the navbar will be “Sign In”.
    1. `complete - meets requirement` Clicking the “Sign In” link will redirect the user to the “Sign In” page.

7. `complete - meets requirement` Clicking the “Find Ingredients” or “About” link will redirect the user to the “Find Ingredients” page or the “About” page, respectively.

8. `complete - meets requirement` Clicking the logo image will redirect the user to the home page, which is also the “Find Ingredient” page.
...
1. Typography
    1. All text on the Bonito website will meet the following standards, as defined by the Web Content Accessibility Guidelines (WCAG):
        1. `complete - meets requirement` The color contrast between all text types and its background will always be 4.5:1 contrast ratio or higher.
        2. `complete - meets requirement` For continuous text that spans more than two lines, the line height will be 1.5 or higher.
    1. `complete - meets requirement` The only typeface used on the website will be Open Sans, however the font weight may change depending on the context.
    2. `complete - meets requirement` There will be no unusual font or text treatments. This specifically includes clipping, distortion, and animation.
    3. `complete - meets requirement` All text must fall within the size range of 12pt to 84pt.

2. Colors
    1. `revised - we added the exception that placeholder text does not need to meet this requirement because, otherwise, it is difficult to discern whether the text is a placeholder. `The color contrast between any text and its respective background will always be 4.5:1 contrast ratio or higher, _with the exception of placeholder text in form input fields._
...
4. Images
    ...
    2. `complete - meets requirement` No images will be enlarged past their original, source file resolution.
    3. `complete - meets requirement` For site loading and rendering purposes, images rendered on the Bonito website cannot have a file size larger than 1 megabyte, and the dimensions cannot have a width and/or height greater than 4000px.
    4. `complete - meets requirement` Image file types can only be .PNG, .GIF, .JPEG, and/or .SVG.
...
4. `complete - meets requirement` If the user is not already signed in to Bonito, users can navigate to the “Sign In” page by clicking the “Sign in” link on the navbar.
...
9. `complete - meets requirement` If the user is not satisfied with the search results, they can start a new search by clicking the “Find ingredients” link on the navbar.
...
## Ingredient page
1. `complete - meets requirement` The ingredient page will list all the user-reported locations will be displayed beneath the page header. 
    1. `complete - meets requirement` The list will span 7/12 the width of the content area.
    2. If there are no reported locations, the message “Phooey. There are no known locations yet” will appear there instead. 
    3. The locations will be listed in order of most “confirmed” votes. If there is a tie, they will be listed in alphabetical order. 
    4. If there are so many reported locations that it exceeds the window height, there will be a scroll bar, and results will overflow vertically.
...
5. `complete - meets requirement` There will be a square map that is 5/12 the width of the content area. It will be located to the right of the list of locations.
    1. Each listed location will be indicated on the map with a marker at the address’s location on the map. The markers will be placed using address data inputted to the Google Maps API, and its behavior would match the Google Maps API default.
...
## About page
1. `complete - meets requirement` The About Us page will have “About Us” as its header, formatted following the Universal Components section guidelines.

2. `revised - we decided to set the image height to be 250px, and the width to be variable, in order to support responsiveness. Based on the screen width, images may occupy multiple rows. This improves readability and user expereince.` Below the header will be all five team members’ photos ~~in a row, with each photo being a 200px by 200px square~~ . _Each image's height will be 250px, and the width will adjust responsively based on screen width. Photos may wrap to multiple rows, depending on the screen width, but rows that are full will always take up 100% of the container width._

3. `complete - meets requirement` Below each member’s photo with will be their first names in bold typeface weight.

4. `complete - meets requirement` Below each member’s first name will also be their job title within the project in defaultly weighted text. Each member’s name and title will be horizontally centrally aligned to their picture.

5. `complete - meets requirement` A description of Bonito as a product and the problem that it is trying to solve will be displayed in a left-aligned paragraph in standard type after one line break below each member’s job titles.
